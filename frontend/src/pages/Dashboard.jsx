...
import { Users,  LifeBuoy, FileText, LayoutDashboard, LogOut } from "lucide-react";
...
export default function Dashboard() {
  ...
  const tabClass = (tab) =>
    `text-lg font-medium py-3 border-b border-gray-300 transition-colors cursor-pointer flex items-center px-3 rounded-md ${
      activeTab === tab ? "bg-blue-100 text-blue-700 font-semibold" : "hover:bg-blue-100 hover:text-blue-700"
    }`;

  if (error) {
    return <div className="text-red-500 text-center mt-5 font-serif">{error}</div>;
  }
  ...
  return (
    <div className="min-h-screen flex bg-gray-100 font-sans">
      ...
      <aside className="w-64 bg-blue-50 border-r-2 border-gray-300 p-4 h-screen space-y-4 shadow-lg">
        <div className={tabClass("dashboard")} onClick={() => setActiveTab("dashboard")}>
          <LayoutDashboard size={20} className="mr-2" />
          Dashboard
        </div>

        {user?.role !== "user" && (
          <div className={tabClass("users")} onClick={() => setActiveTab("users")}>
            <Users size={20} className="mr-2" />
            Users
          </div>
        )}

        <div className={tabClass("tickets")}>
        ...
        <div className="mt-10 text-lg font-medium py-3 border-t border-gray-300 cursor-pointer flex items-center px-3 text-red-600 hover:bg-red-100 rounded-md" onClick={handleLogout}>
        ...
        <div className="relative z-10">
          <div className="bg-gray-800 text-white flex items-center justify-between p-4 shadow-md">
            <div className="flex items-center">
              ...
              {user && <span className="font-serif">Tenant ID: {user.tenantId}</span>}
            </div>
            <img src="/logo.jpeg" alt="Logo" className="h-10" />
          </div>

          {activeTab === "dashboard" ? <DashboardContent user={user} /> : <UserList />}
        </div>
      </div>
    </div>
  );
